...
export interface ForemostOptions {
  configFile?: string;
}

export async function runForemost(
  file: Buffer | NodeJS.ReadableStream,
  options: ForemostOptions = {}
): Promise<string> {
  ...
  try {
    ...
    const args = [
      "exec",
      container,
      "/docker/scripts/run-foremost.sh",
      containerPath,
      options.configFile || ""
    ];
    ...
    throw new Error(`Foremost failed: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
    await Promise.all([
