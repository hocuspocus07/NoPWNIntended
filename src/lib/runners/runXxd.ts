...
export async function runXxd(length: number, file: Buffer | NodeJS.ReadableStream): Promise<string> {
  if (length <= 0 || length > 100000) {
    throw new Error("Invalid length argument for xxd");
  }
  ...
  try {
    ...
    throw new Error(`xxd failed: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
    // Clean up
    await Promise.all([
