...
export interface TestdiskOptions {
  partitionType: string;
}

export async function runTestdisk(
  ...
  try {
    ...
    const args = [
      "exec",
      container,
      "/docker/scripts/run-testdisk.sh",
      containerPath,
      options.partitionType || "auto"
    ];

    const { stdout } = await asyncExecFile("docker", args);
    return stdout;
  } catch (error) {
    throw new Error(`Testdisk failed: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
    await Promise.all([
